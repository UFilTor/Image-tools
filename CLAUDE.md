...
- **Upscale** (`/upscale`) — in-browser AI super-resolution (UpscalerJS + tfjs, no API). Inference runs in a web worker (`lib/upscale.worker.ts`, raw RGB buffers in / RGBA out, tensor-only inside — workers have no Image/document). Upscale mode uses esrgan-thick (best quality; only viable off the main thread), enhance mode uses esrgan-medium (its sources are much larger). If the worker can't start, `lib/upscale.ts` falls back to main-thread esrgan-medium. Sources under 1000px (longest side) default to upscale 2×/4×; larger sources default to "Enhance" — a supersample round-trip (2× up, high-quality downscale back + mild unsharp mask) that keeps dimensions but sharpens/denoises. The default is overridable per image via the ModeToggle in the compare view (only size-cap-ineligible modes are disabled). Transparency is preserved: alpha is resized separately and re-applied; before inference, opaque RGB is bled into transparent regions (`lib/upscale-pixels.ts`) so the model doesn't create dark halos. Do NOT swap in MAXIM models for enhance: they exceed WebGL texture limits or hard-freeze the main thread in-browser (tested). Do NOT use esrgan-thick on the main thread (hangs the tab). Output goes through raw pixels → `canvas.toBlob` object URLs, never base64 (sync base64 encode froze the page). Tiling uses patchSize 128 / padding 8 (padding 2 is the documented minimum before seams show). Size guards in `lib/upscale.ts` cap output at 40MP / 10000px per side.
